...
c
     ...
     &                 rgso(NLU,5),diadep(nsecdep),wfdep(nsecdep)

c
c      real diadep(nsecdep)    ! log-mean diameter for each bin
c      real wfdep(nsecdep)     ! weighting-factor for each bin
c
c     wfdep(1) + ... + wfdep(nsecfin) = 1
c     wfdep(nsecfin+1) + ... + wfdep(nsecdep) = 1
c
c      data diadep /0.055013, 0.12847,     0.3, 0.70057,  1.6360,
c     &               3.8204,  8.9214,  20.833/
c      data wfdep  /  0.0294,  0.2094,  0.4061,  0.2216,  0.1335,
c     &               0.4078,  0.4205,  0.1717/
c
