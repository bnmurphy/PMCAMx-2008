...
c      implicit none
      ...
      data cyr /'2060'/ !'2056','2058' done,'1996'/'2000' done in add182.m, and '2060' later
...
c
      do idate=1,simdays
      if (cdate(idate) .eq. '182') then !skip everything on the first day for leap years
       print *, 'Skipping day ',cdate(idate)       !or skip 183 for 1997
         GO TO 900
      end if
...
c            if (cdate(idate) .eq. '182') then
            if (idate .eq. 2) then
             write( (ifout+io), *),cdate(idate),j,i,0
            else
