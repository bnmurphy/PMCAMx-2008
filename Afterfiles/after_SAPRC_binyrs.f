...
c      parameter(ifout3 = 29)
      ...
      parameter(mspc = 52)    ! Number of output species
      parameter(agg  = 0)     ! Number of Aggregate Species
      parameter(simdays = 31)  ! Num of Simulation Days
...
c     &		   pname(io)(1:isblk-1)//'.daily' !temporarily commented out for speed
...
c	     fname = outdir(1:INDEX(outdir,' ')-1)//
c     &		   scenario(1:INDEX(scenario,' ')-1)//'/'//
c     &		   aggname(io-mspc)(1:isblk-1)//'.daily'
...
c
	...
	do io=1,(mspc+agg)
	if (io.le.mspc) then 
	print *, pname(io), avgconh(io)/simdays/nyr,'\n'
	write( ifout2, *), pname(io), avgconh(io)/simdays/nyr, avgconh1(io)/simdays/nyr,
     &                     avgconh2(io)/simdays/nyr
c	write( ifout3, *), pname(io), avgconh2(io)/simdays/nyr
	else 
	print *, aggname(io-mspc), avgconh(io)/simdays/nyr,'\n'
	write( ifout2, *), aggname(io-mspc),avgconh(io)/simdays/nyr,avgconh1(io)/simdays/nyr,
     &                     avgconh2(io)/simdays/nyr
c	write( ifout3, *), aggname(io-mspc), avgconh2(io)/simdays/nyr
	end if
	end do
