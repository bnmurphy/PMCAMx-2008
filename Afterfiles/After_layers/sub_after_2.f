      subroutine  after(indir,filename,idate,ndate,nhr,nlay,cspc,mconc,
...
c
      ...
      character*198  indir
      real   	     UOMOC, ROMOC, UOMOCa, ROMOCa
...
c                  ***** Gases *****
     ...
     &			    + conh(isp,khr,i,j,1)
	      ...
	      elseif (cspc(kspc).eq.'NOx'.and.(spcname(isp).eq.'NO'.or.spcname(isp).eq.'NO2')) then
		do i = 1,ncol
		  do j = 1,nrow
		    do khr = 1,nhr
		    sconh(khr,i,j,kspc) = sconh(khr,i,j,kspc)
