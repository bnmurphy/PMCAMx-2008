...
c     New OA SPECIATION WITH 8 BINS AND NT-SOA;   mspc = 128, mspc reduced = 115
c      data cspc /'NO',  'NO2', 'O3', 'PTOC1','PTOC0', !'RNO3','HNO4','HONO',
...
c        mspc = 114
     ...
     &           'TotNH3','TotNO3',   'SOA', 'POA', 'TotOM','PM25','NOx',
...
c
      ...
      read(*,*), scenario
      indir    = '/home/bnmurphy/Research/Output/'//fname(1:INDEX(fname,' ')-1)//'/'
      outdir   = '/home/bnmurphy/Research/processed_output/'//fname(1:INDEX(fname,' ')-1)//'/'
...
c
        call after(indir1, fname1, idate, ndate, nhr, nlay, cspc, 
...
c
      if (tasks(1).eq.1) then
        ...
        call after_saprc(outdir,scenario,nlay,clay,ilay,ndate,cdate,mspc,cspc,nspc,mconc)
      endif
      if (tasks(2).eq.1) then
        print *,'TALLYING DIURNAL PROFILES FOR AVERAGE SPECIES AT SPECIFIC SITES'
        call after_di_saprc(outdir,scenario,nlay,clay,ilay,ndate,cdate,mspc,cspc,nspc,mconc)
      end if
      if (tasks(3).eq.1) then
        print *,'Calculating Diurnal Profile for Domain Map'
        call after_dimap_saprc(outdir,scenario,nlay,clay,ilay,ndate,cdate,mspc,cspc,nspc,mconc)
      end if
