...
c      parameter(ifout = 30)
      ...
      real*4 avgconh0(mspc+agg), avgconh1(mspc+agg), avgconh2(mspc+agg)
      real*4 avgconh3(mspc+agg)
      real*4 date1
      
      integer*4 ione,nspec,ibdate,iedate,idum,izero,nx,ny,nz
      integer*4 itbdate,col,row
      integer*4 ihr,msec,icut,line
      integer*4 iday,ix,iy,  lay, height

      data cdate /'195','196','197','198','199','200',
...
c     &		  'ABS1','ABS2','ABS3','ABS4','CBS1','CBS2','CBS3','CBS4',
...
c     &		  'COO1','COO2','COO3','COO4','COO5','COO6','COO7','COO8','COO9'/
...
c
      ...
      scenario2 = 'PMCAMx2008_biogenic50_p5_ht'      !input folder
...
c
c Begin looping over layers (see below if just avg files)
c
...
c             fname = outdir(1:INDEX(outdir,' ')-1)//
c     &		   scenario(1:INDEX(scenario,' ')-1)//'/'//
c     &		   pname(io)(1:isblk-1)//'.daily'//clay(ilay)

             fname2 = outdir(1:INDEX(outdir,' ')-1)//
     &             scenario(1:INDEX(scenario,' ')-1)//'/'//
     &              pname(io)(1:isblk-1)//'.avg'
...
c	     fname = outdir(1:INDEX(outdir,' ')-1)//
c     &		   scenario(1:INDEX(scenario,' ')-1)//'/'//
c     &		   aggname(io-mspc)(1:isblk-1)//'.daily'//clay(ilay)
	  
             fname2 = outdir(1:INDEX(outdir,' ')-1)//
     &             scenario(1:INDEX(scenario,' ')-1)//'/'//
     &             aggname(io-mspc)(1:isblk-1)//'.avg'
	  end if
        
c	  isblk  = INDEX(fname,' ')
	  isblk2 = INDEX(fname2,' ')
c	  open(unit=(ifout+io),file=fname(1:isblk-1),status='UNKNOWN')
          open(unit=(ifout2+io),file=fname2(1:isblk2-1),status='UNKNOWN')

c	  if (ilay.eq.1) then
	  write(unit=(ifout2+io),*),'Content loading, wait until end of run'
	  close(unit=(ifout2+io))
          open(unit=(ifout2+io),file=fname2(1:isblk2-1),status='UNKNOWN')
	  write(unit=(ifout2+io),fmt=47),'Compound ','Layer ','Avg ',
     &		'PGH ','CIN ','HAT ','ATL '
c	  end if 
	end do
...
c        hfile = '/home/mday/processed_output/height/PandH.avg'
c        read(hfile,*)lay,pres,height
...
c	    do ilay = 1,nz
     ...
     &		  ((conh(isp,ihr,i,j,1),i=1,nx),j=1,ny)
...
c          sconh(83,i,j) = sconh(83,i,j)*(1.0133*10**2*180/(8.314*300))
c          sconh(84,i,j) = sconh(84,i,j)*(1.0133*10**2*180/(8.314*300))
c          sconh(85,i,j) = sconh(85,i,j)*(1.0133*10**2*180/(8.314*300))
c          sconh(86,i,j) = sconh(86,i,j)*(1.0133*10**2*180/(8.314*300))
c       CAS from ppb -> ug/m^3
c          sconh(91,i,j) = sconh(91,i,j)*(1.0133*10**2*150/(8.314*300))
c          sconh(92,i,j) = sconh(92,i,j)*(1.0133*10**2*150/(8.314*300))
c          sconh(93,i,j) = sconh(93,i,j)*(1.0133*10**2*150/(8.314*300))
c          sconh(94,i,j) = sconh(94,i,j)*(1.0133*10**2*150/(8.314*300))
c       CPO from ppb -> ug/m^3
c          sconh(104,i,j) = sconh(104,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(105,i,j) = sconh(105,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(106,i,j) = sconh(106,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(107,i,j) = sconh(107,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(108,i,j) = sconh(108,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(109,i,j) = sconh(109,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(110,i,j) = sconh(110,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(111,i,j) = sconh(111,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(112,i,j) = sconh(112,i,j)*(1.0133*10**2*250/(8.314*300))
c       COO from ppb -> ug/m^3
c          sconh(122,i,j) = sconh(122,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(123,i,j) = sconh(123,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(124,i,j) = sconh(124,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(125,i,j) = sconh(125,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(126,i,j) = sconh(126,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(127,i,j) = sconh(127,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(128,i,j) = sconh(128,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(129,i,j) = sconh(129,i,j)*(1.0133*10**2*250/(8.314*300))
c          sconh(130,i,j) = sconh(130,i,j)*(1.0133*10**2*250/(8.314*300))
c       TotNH4 = PNH4 + NH3
...
c            write( (ifout+io), *),cdate(idate),j,i,sconh(io,i,j)/nhr
            maxh(io) = AMAX1(maxh(io),sconh(io,i,j)) 
		avgconh(io) = avgconh(io) + sconh(io,i,j)/nhr/nx/ny !for all sites
          end do
        end do
	
	avgconh0(io) = avgconh0(io) + sconh(io,65,51)/nhr !for Pittsburgh, PA
	...
	avgconh2(io) = avgconh2(io) + sconh(io,46,20)/nhr !for Hattiesburg, MS
        avgconh3(io) = avgconh3(io) + sconh(io,58,29)/nhr !for Atlanta, GA
...
c        if (ilay.eq.1) then
c        write(ifout2+io,*),'Compound ','Layer ','GridAverage ','Pittsburgh,PA ',
c     &                  'Cincinnati,OH ','Hattiesburg,MS'
c        end if

	if (io.le.mspc) then 
	print *, pname(io),clay(ilay),avgconh(io)/simdays,avgconh0(io)/simdays,
     &		avgconh1(io)/simdays,avgconh2(io)/simdays,avgconh3(io)/simdays,'\n'
	write( (ifout2+io), fmt = 48), pname(io), clay(ilay), avgconh(io)/simdays,
     &		 avgconh0(io)/simdays, avgconh1(io)/simdays, avgconh2(io)/simdays
     & 		 , avgconh3(io)/simdays
	else 
	print *, aggname(io-mspc), avgconh(io)/simdays,'\n'
	write( (ifout2+io), fmt = 48), aggname(io-mspc), clay(ilay),
     &		 avgconh(io)/simdays, avgconh0(io)/simdays, avgconh1(io)/simdays 
     &		 , avgconh2(io)/simdays, avgconh3(io)/simdays
...
c	do io = 1,(mspc+agg)
c		close ((io+ifout))	!Close Output Files
c		close ((io+ifout2))
c	end do
...
c  47   format(i3,1x,i2,1x,i2,1x,f10.5)
  47   format(a9,1x,a6,1x,a4,1x,a4,1x,a4,1x,a4,1x,a4)
  48   format(a5,1x,a2,1x,f9.6,1x,f9.6,1x,f9.6,1x,f9.6,1x,f9.6)
c  48   format(a5,a2,f8.6,f8.6,f8.6,f8.6,f8.6)
