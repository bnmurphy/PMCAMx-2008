...
c     This version outputs Gas, PM 2.5 (&NOT PM10) in Pittsburgh PER HOUR
c######################################################################
...
c      implicit none
      ...
      real*4 sconh(mspc+agg,nhr,ncol,nrow)  !Conc for output
      real*4 date1
      
      integer*4 ione,nspec,ibdate,iedate,idum,izero,nx,ny,nz
      integer*4 itbdate,col,row
      integer*4 ihr,msec,icut,line,khr,t
...
c
      ...
      scenario = 'PMCAMx2008_BASE_sulf'   !output folder
      scenario2 = 'PMCAMx2008_BASE_sulf'  !input folder
...
c
	...
	do io = 1,(mspc+agg)
     ...
     &		   pname(io)(1:isblk-1)// '.atl.hourly'          !'.pit.hourly'
	  else				!Aggregate Species
	     isblk = INDEX(aggname(io-mspc),' ')
	     if (isblk.eq.0) isblk = 7
	     fname = outdir(1:INDEX(outdir,' ')-1)//
     &		   scenario(1:INDEX(scenario,' ')-1)//'/'//
     &		   aggname(io-mspc)(1:isblk-1)// '.atl.hourly'   !'.pit.hourly'
	  end if
...
c
      print *,'Reading Hourly Concentrations...'
      do isp = 1,nspec
...
c                  ***** Gases *****
     ...
     &		  pname(kspc)(1:INDEX(pname(kspc),' ')-1)) then
            	do khr = 1,nhr
                  do i = 1,nx
            	  do j = 1,ny
		    sconh(kspc,khr,i,j) = sconh(kspc,khr,i,j) 
...
c	TotNH4 = PNH4 + NH3
      	sconh(79,t,i,j) = sconh(76,t,i,j)+sconh(61,t,i,j)*(1.0133*10**2*17/(8.314*300))
c	TotNO3 = PNO3 + HNO3	
	sconh(80,t,i,j) = sconh(77,t,i,j)+sconh(35,t,i,j)*(1.0133*10**2*63/(8.314*300))
c	CIS (currently refers to CPO?)
	  sconh(62,t,i,j) = sconh(62,t,i,j)*(1.0133*10**2*250/(8.314*300))
c	CR1 (currently refers to COO?)
          sconh(63,t,i,j) = sconh(63,t,i,j)*(1.0133*10**2*250/(8.314*300))
c       SOA = ABS + AAS        
          sconh(81,t,i,j) = sconh(68,t,i,j) + sconh(69,t,i,j)
c	POA = APO + AOO + POC      
	  sconh(82,t,i,j) = sconh(66,t,i,j) + sconh(67,t,i,j) + sconh(70,t,i,j)
c	TotOM = SOA + POA
	  sconh(83,t,i,j) = sconh(81,t,i,j) + sconh(82,t,i,j)

c       cSOA = ABS + AAS with OM->OC conversion factors      
          sconh(84,t,i,j) = sconh(68,t,i,j)/2 + sconh(69,t,i,j)/2
c       cPOA = APO + AOO + POC with OM->OC conversion factors      
          sconh(85,t,i,j) = sconh(66,t,i,j)/1.4 + sconh(67,t,i,j)/2 + sconh(70,t,i,j)/2
c       TotOC = cSOA + cPOA
          sconh(86,t,i,j) = sconh(84,t,i,j) + sconh(85,t,i,j)

c	PM2.5 = OM + PEC/2 + CRST + PCL + NA + ~PH2O + PNH4 + PNO3 + PSO4        
	  sconh(87,t,i,j) = sconh(83,t,i,j) + sconh(71,t,i,j)/2 + sconh(72,t,i,j)
     &                  + sconh(74,t,i,j) + sconh(75,t,i,j)
     &		        + sconh(76,t,i,j) + sconh(77,t,i,j) + sconh(78,t,i,j)
...
c
      ...
      do io = 1,(mspc+agg)
       do t=1,nhr 
c	do j=1,ny     ! ROW LATER
c          do i=1,nx   ! COLUMN FIRST
            write( (ifout+io), *),cdate(idate),t,sconh(io,t,58,29) !don't forget to change filename! 
            !65/51 for Pittsburgh, 52/27 Birmingham, 79/55 NYC, 47/52 Chicago, 58/29 Atlanta
c            maxh(io) = AMAX1(maxh(io),sconh(io,i,j))
c          end do !i
c        end do   !j
       end do    !hr
      end do     !io
